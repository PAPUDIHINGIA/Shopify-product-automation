...
def extract_variants(page, base_price: str, sels: dict) -> list[dict]:
    """
    UPGRADED: now handles multi-option products (Size + Color = cartesian rows).

    Shopify requires one row per variant combination. If a product has
    Size: S/M/L and Color: Red/Blue, that is 6 rows not 2.
    This function detects all option axes and builds the full combination set.

    Priority order:
    1. Shopify product JSON  (most reliable — has price + SKU per variant)
    2. WooCommerce JSON      (for WooCommerce stores)
    3. DOM fallback          (generic — extracts option values from selects/radios)
    """
    variants = []
    ...
    try:
        ...
        if product_data and isinstance(product_data, dict):
            ...
            # UPGRADE: Shopify supports up to 3 option axes.
            # We map option1/option2/option3 → the named option columns.
            # For now we export Option1 (required) — extend here for Option2/3.
            option1_name = options[0] if len(options) > 0 else "Title"

            for v in raw_variants:
                raw_p = v.get("price", 0)
                price = f"{int(raw_p) / 100:.2f}" if raw_p else base_price

                # Build a descriptive value combining all option axes
                # e.g. "Small / Red" instead of just "Small"
                opt_parts = [
                    v.get("option1", ""),
                    v.get("option2", ""),
                    v.get("option3", ""),
                ]
                opt_value = " / ".join(p for p in opt_parts if p)

                variants.append({
                    "name":  option1_name,
                    "value": opt_value or v.get("title") or "Default",
                    "sku":   v.get("sku", ""),
                    "price": price,
                })
    ...
    try:
        ...
        if woo_data and isinstance(woo_data, list) and len(woo_data) > 0:
            for v in woo_data:
                ...
                attr_name = (
                    attr_keys[0].replace("attribute_pa_", "").replace("attribute_", "").title()
                    if attr_keys else "Option"
                )
                # UPGRADE: combine all attribute values for multi-option WooCommerce
                attr_val = " / ".join(
                    str(val) for val in attrs.values() if val
                ) or "Default"
                variants.append({
                    "name":  attr_name,
                    "value": attr_val,
...
def scrape_product_page(
    ...
    for attempt in range(1, max_retries + 2):   # 1 … max_retries+1 total tries
        product_page = None
        try:
            product_page = new_stealth_page(browser)  # open a fresh tab per product
            ...
            time.sleep(delay)  # honour the --delay flag
...
def extract_cards(catalog_page, browser, base_url, sels, skip_handles,
...
def scrape_next_button(browser, base_url, next_sel, sels, skip_handles,
    ...
    while True:
        ...
        batch, first_flush, total_written, hit_limit = extract_cards(
            catalog_page, browser, base_url, sels, skip_handles,
            batch, first_flush, total_written, batch_size,
            output_path, review_path, limit, page_num, use_product_page,
...
def scrape_url_pattern(browser, url_pattern, page_start, page_end, sels, skip_handles,
    ...
    for page_num in range(page_start, page_end + 1):
        ...
        batch, first_flush, total_written, hit_limit = extract_cards(
            catalog_page, browser, url, sels, skip_handles,
            batch, first_flush, total_written, batch_size,
            output_path, review_path, limit, page_num, use_product_page,
            stats=stats, delay=delay, max_retries=max_retries,
        )
...
if __name__ == "__main__":
    ...
    with sync_playwright() as pw:
        browser = pw.chromium.launch(
            # UPGRADE: --show-browser runs visible window for debugging
